...
public class VentanaApp extends JFrame {
	...
	private JTable getTablaCarrerasParticipante() {
		if (tablaCarrerasParticipante == null) {
			...
			tablaCarrerasParticipante.setModel(new DefaultTableModel(new Object[][] {}, new String[] { "Carrera" }));
		}
	...
	private JTable getTablaParticipantesClub() {
		if (tablaParticipantesClub == null) {
			...
			tablaParticipantesClub.setModel(new DefaultTableModel(new Object[][] {},
					new String[] { "E-mail", "Dni", "Nombre", "Fecha nacimiento", "Sexo" }) {
				private static final long serialVersionUID = 1L;
	...
	private JComboBox<String> getCbSexoAtletaClub() {
		if (cbSexoAtletaClub == null) {
			...
			cbSexoAtletaClub.setModel(new DefaultComboBoxModel(new String[] { "hombre", "mujer" }));
...
//				}
	...
	private void comprobarPuedePagar(String id, String email) {
		if (puedePagar(id, email)) {
			if ( estaATiempo(id, email)) {
				...
				JOptionPane.showMessageDialog(this, "Fecha tard�a para el pago de la carrera: " + id);
			}
		} else {
	...
	private void a�adirParticipanteClub() {
		if (Validadores.comprobarEmail(getTxEmailAtletaClub().getText())
				...
				&& Validadores.comprobarMayor18(getTxFechaNacimientoAtletaClub().getText())) {
			if (compruebaEmailListaClub(getTxEmailAtletaClub().getText())) {
				System.out.println();
				JOptionPane.showMessageDialog(this, "El atleta ya se enecuentra en la lista", "Error",
						JOptionPane.WARNING_MESSAGE);
				return;
			}
			Object[] obj = new Object[] { getTxEmailAtletaClub().getText(), getTxDniAtletaClub().getText(),
					getTxNombreAtletaClub().getText(), getTxFechaNacimientoAtletaClub().getText(),
					getCbSexoAtletaClub().getSelectedItem().toString() };
			((DefaultTableModel) tablaParticipantesClub.getModel()).addRow(obj);
			vaciarCamposClub();
		} else {
	...
	private void vaciarCamposClub() {
		getTxEmailAtletaClub().setText("");
		getTxDniAtletaClub().setText("");
		getTxNombreAtletaClub().setText("");
		getTxFechaNacimientoAtletaClub().setText("yyyy/mm/dd");
	}
